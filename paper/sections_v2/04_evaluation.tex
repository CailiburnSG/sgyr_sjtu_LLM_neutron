...
Three non-pooled protocol families are used. As summarized in the opening overview (\cref{fig:evaluation-design,tab:retrieval-protocols}), the historical \texttt{nomic-embed-text} index supplies the broadest legacy robustness and self-retrieval results. The primary query-formulation-by-chunking comparison, hereafter P0, reuses the historical character-based chunking regime at three scopes and evaluates two MiniLM encoders with both baseline and manually authored technical-detail queries. Here, P0 is an experiment-priority label rather than a model or processing stage. The 240-word MiniLM setting is retained last as a fixed-word cross-check, rather than being treated as the reference scale for the other protocols.
