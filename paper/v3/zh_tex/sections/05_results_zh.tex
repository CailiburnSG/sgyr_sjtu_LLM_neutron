...
在保持资料库、top-10 规则和来源指标不变的 MiniLM 核查中，以两种 encoder 和三种字符分块设置比较简短基线 query 与由 observation packet 线索形成的技术细节 query。图~\ref{fig:query-effect} 的每个点表示固定 encoder--分块设置下，技术细节 query 家族相对基线 query 家族的平均 CorePriority@10 变化。在此口径下，英文设置的变化介于 $-0.03$ 至 $+0.14$，中文设置介于 $-0.23$ 至 $+0.02$。因此，向 query 中加入观察细节并非总能提高核心来源可见性。
...
\begin{figure}[H]
  ...
  \end{minipage}
  \caption{加入 10 份补充资料时，技术细节 query 相对基线 query 的平均 CorePriority@10 变化。每点对应一个 encoder--字符分块设置；正值表示技术细节 query 家族的核心来源可见性（CorePriority@10）较高。(a) 英文设置；(b) 中文设置。}
