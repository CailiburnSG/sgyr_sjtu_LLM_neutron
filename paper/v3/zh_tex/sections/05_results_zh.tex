...
这一结果不表示补充资料不相关。相反，它表明当部署要求核心权威来源保持可见时，相关补充资料本身就足以改变无来源约束语义检索的结果构成。图~\ref{fig:scope-priority} 同时呈现核心来源可见性和最高余弦相似度随扩库规模的轨迹，使读者直接看到“相似度提高或稳定”与“核心来源可见性下降”可以同时发生。

\begin{figure*}[!t]
  \centering
  \begin{minipage}[t]{0.47\textwidth}
    \centering\textbf{(a)}\par\vspace{1mm}
    \includegraphics[width=\linewidth]{fig05a_scope_priority}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.47\textwidth}
    \centering\textbf{(b)}\par\vspace{1mm}
    \includegraphics[width=\linewidth]{fig05b_top1_cosine}
  \end{minipage}
  \caption{字符分块扩库下的语义相似度与核心来源可见性。结果聚合 17 个已保存分块配置；阴影表示配置间标准差。(a) CorePriority@10 可随相关补充资料加入而下降；(b) 在相同协议中，平均最高余弦相似度提高或保持稳定。两者可同时发生。}
...
在两种 MiniLM encoder 分别配合三种字符分块设置所形成的六种配置上，平均结果也体现了这种不对称性：英文技术细节 query 的 CorePriority@10 为 0.65，高于英文基线 query 的 0.60；中文技术细节 query 为 0.52，低于中文基线 query 的 0.62。完整扩库轨迹进一步显示，各曲线均从 core-only 条件下的 1.00 出发，但其下降速度、交叉位置和对 query 细节的响应随语言与配置而变。图~\ref{fig:configuration-interaction} 以一张热图压缩呈现这一交互，不再在正文同时保留多张语言重复图、梯度图和轨迹图。

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.52\textwidth]{fig05c_configuration_heatmap}
