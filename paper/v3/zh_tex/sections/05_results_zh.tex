...
这一 trace 的关键不在于事件数本身，而在于每个摘要字段均可回指：零值时间、事件计数和持续时间来自事件扫描输出；通道间共变、极值位置和快照来自相应的确定性工具；condition report 只组织这些已保存字段。本文不以事件计数汇总作为结果图展示，也不将资料检索后的候选说明提升为已确认根因。
...
\begin{figure}[H]
  ...
  \end{minipage}
  \caption{字符分块扩库下的语义相似度与核心资料在前 10 个结果中的占比（来源可见性）。结果聚合 17 个已保存分块配置；阴影表示配置间标准差。(a) CorePriority@10 可随相关补充资料加入而下降；(b) 在相同协议中，平均最高余弦相似度提高或保持稳定。两者可同时发生。}
  \label{fig:scope-priority}
\end{figure}

这一结果不表示补充资料不相关。相反，它表明当部署要求前 10 个结果保留核心权威资料时，相关补充资料本身就足以改变无来源约束语义检索的结果构成。图~\ref{fig:scope-priority} 同时呈现核心资料在前 10 个结果中的占比和最高余弦相似度随扩库规模的轨迹，使读者直接看到“相似度提高或稳定”与“核心资料占比下降”可以同时发生。

\subsection{MiniLM encoder 对照：query、语言与分块的交互}

为避免将中文 query 与英文向 encoder 的语言失配混入主比较，图~\ref{fig:query-effect} 仅保留多语 MiniLM encoder，并在三种字符分块设置下比较简短基线 query 与由 observation packet 线索形成的技术细节 query。每个点表示固定分块设置下，技术细节 query 家族相对基线 query 家族的平均 CorePriority@10 变化。英文设置的变化介于 $+0.05$ 至 $+0.14$，中文设置介于 $-0.23$ 至 $+0.02$。因此，向 query 中加入观察细节并非总能提高核心来源可见性。
...
\begin{figure}[H]
  ...
  \end{minipage}
  \caption{多语 MiniLM encoder 在加入 10 份补充资料时，技术细节 query 相对基线 query 的平均 CorePriority@10 变化。每点对应一个字符分块设置，左图纵轴标出两图共享的分块长度/重叠；正值表示技术细节 query 家族的核心来源可见性（CorePriority@10）较高。}
  \label{fig:query-effect}
\end{figure}
}

在两种 MiniLM encoder 分别配合三种字符分块设置所形成的六种配置上，平均结果也体现了这种不对称性：英文技术细节 query 的 CorePriority@10 为 0.65，高于英文基线 query 的 0.60；中文技术细节 query 为 0.52，低于中文基线 query 的 0.62。完整扩库轨迹进一步显示，各曲线均从 core-only 条件下的 1.00 出发，但其下降速度、交叉位置和对 query 细节的响应随语言与配置而变。图~\ref{fig:configuration-interaction} 以一张热图压缩呈现这一交互，并补充 Fig.~\ref{fig:query-effect} 的家族均值比较。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\columnwidth]{fig06_configuration_heatmap}
  \caption{$m=10$ 时 query、语言、encoder 与字符分块方式的交互。每个单元格为 CorePriority@10 的平均值；横向分隔线区分英文与中文 query，纵向分隔线区分两种 encoder。由 observation packet 线索形成的技术细节 query 并不在所有语言和配置下提高核心来源可见性。}
...
\begin{tabular}{p{0.16\columnwidth} p{0.22\columnwidth} p{0.34\columnwidth} p{0.20\columnwidth}}
\toprule
候选用途 & encoder 与分块 & query 表述 & \shortstack{CorePriority@10\\$m=10$ / $m=51$} \\
\midrule
...
为保留不同扩库终点与检索单元定义下的完整证据，图~\ref{fig:priority-retention} 和图~\ref{fig:self-retrieval} 将相关结果置于本节后段。它们用于展开前述条件性选择和指标边界，不替代前文的主结论图。

\begin{figure*}[!t]
  \centering
  \begin{minipage}[t]{0.29\textwidth}
    \centering\textbf{(a)}\par\vspace{1mm}
    \includegraphics[width=\linewidth]{fig07a_priority_retention}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.31\textwidth}
    \centering\textbf{(b)}\par\vspace{1mm}
    \includegraphics[width=\linewidth]{fig07b_priority_trajectory_en}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.31\textwidth}
    \centering\textbf{(c)}\par\vspace{1mm}
    \includegraphics[width=\linewidth]{fig07c_priority_trajectory_zh}
  \end{minipage}
  \caption{扩库后的来源可见性保持能力：（a）不同配置在 $m=10$ 与 $m=51$ 的对应关系，落在对角线下方表示进一步衰减；（b）英文 query 的完整扩库轨迹；（c）中文 query 的相应轨迹。该图组展开候选配置的条件性选择，不建立跨语言或跨 encoder 的普适排名。}
  \label{fig:priority-retention}
\end{figure*}
...
\begin{figure*}[!t]
  ...
  \begin{minipage}[t]{0.43\textwidth}
    \centering\textbf{(a)}\par\vspace{1mm}
    \includegraphics[width=\linewidth]{fig08a_self_retrieval_recall}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.43\textwidth}
    \centering\textbf{(b)}\par\vspace{1mm}
    \includegraphics[width=\linewidth]{fig08b_self_retrieval_mrr}
  \end{minipage}
  \caption{self-retrieval 的检索单元核查：（a）Recall@10；（b）MRR。两图均区分 exact-chunk 与 same-document 两种匹配标准，用于说明重叠分块下“找回”的指标边界，不衡量工程正确性。}
