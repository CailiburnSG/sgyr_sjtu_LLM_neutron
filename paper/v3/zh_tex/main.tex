% !TeX program = xelatex
% V3 Chinese native LaTeX manuscript. Compile with: xelatex main.tex
\documentclass[UTF8,10pt,a4paper,twocolumn]{ctexart}
\usepackage[margin=2.0cm]{geometry}
\usepackage{amsmath,amssymb,booktabs,longtable,array,graphicx,float,placeins}
\usepackage[numbers,sort&compress]{natbib}
\usepackage{hyperref,xurl}
\hypersetup{colorlinks=true,linkcolor=black,citecolor=blue,urlcolor=blue}
\setlength{\columnsep}{0.75cm}
\graphicspath{{../figs/}{../../v2/figs/}}
\title{证据约束的无标签中子电流时序诊断辅助}
\author{作者信息待补充}
\date{}
\begin{document}
\raggedbottom
\maketitle

\begin{abstract}
待在中文正文与图表稳定后撰写。
\end{abstract}

\noindent\textbf{关键词：}中子仪表；无标签时序；诊断辅助；可追溯观测；检索增强；证据溯源

\input{sections/01_introduction_zh}
\input{sections/02_related_work_zh}
\input{sections/03_framework_zh}
\input{sections/04_evaluation_zh}
\input{sections/05_results_zh}
\input{sections/06_conclusion_zh}

\clearpage
\bibliographystyle{elsarticle-num}
\bibliography{refs}
\end{document}
