...
## 3.2 测量侧：从原始记录到 observation packet
...
初始化完成后，脚本仅执行预先注册的 probes。各 probe 的作用、输出和解释边界如下表所示。

| Probe 家族 | 确定性处理 | 形成的描述性观察 |
|---|---|---|
| 数据健康与初始化 | 时间排序，采样、缺失和不活跃通道检查 | active record、数据健康标记 |
| 形态与共变扫描 | 计算通道变化程度及两两相关关系 | 粗略形态、强共变或近冗余通道组 |
| 事件与区段扫描 | 划分相对稳定区段和短时变化区段，枚举突刺与孤立零值 | 区段与事件清单 |
| 同步与滞后扫描 | 比较通道间的相关—滞后结构 | 近同步组、候选滞后关系 |
| 极值定位 | 定位各通道最大值、最小值及其时刻 | 极值及时间位置 |
| 截面快照 | 在选定时刻记录多通道状态 | 时间局部的通道状态向量 |

形态和共变扫描提供的是信号关系的初步描述。例如，较高的两两相关性可以提示共享变化、可能的冗余或需要结合通道位置进一步核查的情形，但它本身不证明仪表关系或物理机制。相同地，相关—滞后扫描只区分近同步与时间错位的统计模式，不赋予因果方向。
...
这类以中位数和绝对偏差抑制局部异常影响的稳健估计思想，可追溯至相关基础研究 [@hampel1974influence; @rousseeuw1993alternatives]。对于非恒定通道，突刺候选定义为同时满足局部与全程幅度条件的点：
...
技术证据资源由核仪表与控制相关资料构成，并为每份文档保留来源 metadata，以支持段落定位和后续引用。当前资料库包含 64 份文档，其中 13 份构成以 IAEA 在线监测和仪表通道评估资料为主的权威核心，其余为 NRC、EPRI/DOE、OECD/NEA 及实验室或专题报告等同领域补充资料。在线监测与仪表通道评估的工程背景由 IAEA 指南和相关综述提供 [@iaea2008olm1; @iaea2008olm2; @hashemian2011online]。
...
| 摘要中的观察线索 | 检索意图 | 优先查找的证据 |
|---|---|---|
| 突刺或孤立零值 | 信号完整性和异常测量背景 | 定义、核查步骤、诊断注意事项 |
...
| 风险 | 运行约束 | 保留的关联产物 |
|---|---|---|
| 输入或状态漂移 | 仅初始化步骤接收原始路径，之后均解析同一 active record | active-record 标识与 probe 日志 |
| 未预先定义或格式错误的动作 | 只接受预先定义的分析工具；不符合要求的动作不执行 | 接受或拒绝记录 |
| 长叙述直接驱动检索 | condition report 留给人工复核，检索仅读取 compact alert summary | 报告—摘要链接 |
| 缺乏证据的文字解释 | memo 中每项候选表述关联到检索段落和引文 | evidence-linked memo |
...
<!-- 起草备注：42 条记录、四套仪表组、每条 7 个通道、约 6×10^5 样本和约 1 s 采样间隔须在正式 TeX 稿前与数据清单再次核实；反应堆/探测器类别的可披露范围待确认。 -->
